...
- Pen-testing: a second independant tree with a different network card attached to the root
- Intranet firewall: one of the ApplicationVM is configured to have a PCI network card attached (patched to your intranet)
...
A qubes has the network interface eth0 connecting to its NetworkVM while vifXX.0 interface(s) connects it to the other downstream qubes. Physical connected interface have other names depending on their type and other factors.
...
The routing is kept local between a qube and its NetworkVM (no route is present) while allowing allow nodes in the tree to communicate upstream to the NetworkVM sys-net. This is achieved by each qube having his NetworkVM as default gateway while SNATting (masquerading) the traffic.
...
Note that un-masquarading is provided for free when the traffic is initiated from downstream qubes (i.e. AppVM), as it is return (established) traffic. However hosting a service in Qubes requires natting (pre-routing) of the (non established) traffic.
